...
*Setup instructions for a Gitian build of Birch Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Birch
...
- [Building Birch Core](#building-birch-core)
...
Clone the git repositories for Birch Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/birchpay/birch
```
...
in the Birch Core repository under 'Fetch and build inputs' to install sources which require
...
To build Birch Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Birch Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/birch/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/birchpay/birch
...
For example:
```bash
URL=https://github.com/crowning-/birch.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit birch=${COMMIT} --url birch=${URL} ../birch/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit birch=${COMMIT} --url birch=${URL} ../birch/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit birch=${COMMIT} --url birch=${URL} ../birch/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the birch git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../birch/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/birch.git
SIGPATH=/some/root/path/birch-detached-sigs.git

./bin/gbuild --url birch=${BTCPATH},signature=${SIGPATH} ../birch/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/birch-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/birch-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/birch-osx-build.assert
```
...
[birch/gitian.sigs](https://github.com/birchpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
